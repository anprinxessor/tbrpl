	...
	function newJurusan(){
		...
		$('#fm').form('clear');
		url = 'controller/crud.php?tabel=jurusan&tipe=add';
	}
	...
	function editJurusan(){
		...
		if (row){
			...
			$('#fm').form('load',row);
			url = 'controller/crud.php?tabel=jurusan&tipe=update&jurusan_id='+row.jurusan_id;
		}
		else{
	...
	function deleteJurusan(){
		var row = $('#dg').datagrid('getSelected');
		if (row){
			$.messager.confirm('Konfirmasi','Yakin menghapus data jurusan ini?',function(r){
				if (r){
					$.post('controller/crud.php?tabel=jurusan&tipe=delete',{jurusan_id:row.jurusan_id},function(result){
						if (result.success){
							$('#dg').datagrid('reload');	// reload the user data
						} else {
	...
	function refreshJurusan(){
		$('#dg').datagrid('load',{});
	}
	</script>
	
	<!-- Tabel Data-->
	<table id="dg" title="Tabel Data Jurusan" class="easyui-datagrid" style="width:400px;height:auto"
			url="controller/crud.php?tabel=jurusan&tipe=view"
			toolbar="#toolbar" pagination="true"
			rownumbers="true" fitColumns="true" singleSelect="true">
		<thead>
			<tr>
				
				<th field="jurusan_nama" width="auto">Nama</th>
				<th field="fakultas_nama" width="auto">Fakultas</th>
			</tr>
		</thead>
	</table>
	<div id="toolbar">
		<a href="#" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="newJurusan()">Tambah</a>
		<a href="#" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="editJurusan()">Ubah</a>
		<a href="#" class="easyui-linkbutton" iconCls="icon-cancel" plain="true" onclick="deleteJurusan()">Hapus</a>
		<a href="#" class="easyui-linkbutton" iconCls="icon-search" plain="true" onclick="searchJurusan()">Cari</a>
		<a href="#" class="easyui-linkbutton" iconCls="icon-reload" plain="true" onclick="refreshJurusan()">Refresh</a>
	</div>
	
	<!-- Form untuk insert dan update-->
	<div id="dlg" class="easyui-dialog" style="width:420px;height:auto;padding:10px 20px"
			closed="true" buttons="#dlg-buttons">
		<div class="ftitle">JURUSAN</div>
		<form id="fm" method="post" novalidate>	
			
			<div class="fitem">
				<label>Nama</label>
				<input name="jurusan_nama" class="easyui-validatebox" required="true"/>
			</div>
